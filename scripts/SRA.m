...
clc; clear; close all
folder = '~/ROS/DeepEnd2End/DeepToF_release_0.1/src/GAN/datasets/mpi_correction_corrnormamp2depth_albedoaug_zpass_test/test/';
fn = dir([folder '*.mat']);
folder_out = 'SRA_results_all/';
mkdir(folder_out);
isnoise = 0;
isvis = 0;
...
for ifn = 1:numel(fn) %[170,69,86,110,129]
    ...
    pointCloudSRA = depthToPointCloud(distSRA,'pbrt');
    depthSRA = pointCloudSRA(:,:,3)';
    pointCloudGT = depthToPointCloud(dist_gt,'pbrt');
