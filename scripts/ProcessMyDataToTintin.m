    ...
    %         depth = medfilt2(depth); % smooth it a bit
        ...
        for ifreqs = 1:numel(freqs)
            ...
            data_mean = mean(my_data(:,end-navg+1:end),2);
                ...
                phase2dist = 3e8/(2*ex_freqs(ifreqs))
                ex_phase = mod(depth/phase2dist,1) * 2 * pi;
                I_Mat = Amp .* cos(ex_phase);
                Q_Mat = Amp .* sin(ex_phase);

                I_Mat = medfilt2(I_Mat,[medfilt_size,medfilt_size]);
                Q_Mat = medfilt2(Q_Mat,[medfilt_size,medfilt_size]);
                
                Amp2 = abs(I_Mat+1i*Q_Mat);
                Phase2 = angle(I_Mat+1i*Q_Mat);
                if resize_height ~= height
                    ...
                    I_Mat = double(double(I_Mat)./100-1000);
                    Q_Mat = double(double(Q_Mat)./100-1000);
                end
                
                if is_visualizing
                    figure(ifreqs+1); 
                    suptitle(fnc);
                    subplot(321); imagesc(I_Mat); axis image; colorbar; title('I');
                    subplot(322); imagesc(Q_Mat); axis image; colorbar; title('Q');
                    subplot(323); imagesc(Amp); axis image; colorbar; title('amp');
                    subplot(324); imagesc(Phase); axis image; colorbar; title('phase');
                    subplot(325); imagesc(Amp2); axis image; colorbar; title('Amp2');
                    subplot(326); imagesc(Phase2); axis image; colorbar; title('Phase2');
                end
                date{idate}
                takes(itakes)
                freqs(ifreqs)
