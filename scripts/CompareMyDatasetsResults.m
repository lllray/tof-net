...
time_stamp = '1651213921040147';
    ...
    sprintf('~/ROS/DeepEnd2End/DeepToF_release_0.1/src/GAN/results/%s/%s/images/%s/',model_name_gan,data_name,type)
    testList = dir(sprintf('~/ROS/DeepEnd2End/DeepToF_release_0.1/src/GAN/results/%s/%s/images/%s/',model_name_gan,data_name,type));
    n=length(testList);
    for i=3:n
        if run_single == 0
