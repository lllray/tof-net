...
-- fixed rand seed for comparisons
...
if opt.continue_train==1 and opt.continue_from_batch>opt.niter then
    opt.lr = opt.lr - opt.lr/opt.niter_decay*(opt.continue_from_batch-opt.niter)
end
...
function save_current_results(epoch, counter)
  ...
  for i,visual in ipairs(visuals) do
    if opt.mat == 0 then
      ...
      output_path = paths.concat(opt.visual_dir, 'train_epoch' .. epoch .. '_iter' .. counter .. '_' .. visual.label .. '.mat')
    end
...
function plot_current_errors(epoch, counter_ratio, opt)
  ...
  -- os.execute('convert -density 200 '..opt.checkpoints_dir..'/'..opt.name..'/errs_plot.eps '..opt.checkpoints_dir..'/'..opt.name..'/errs_plot.png')
end
...
local epoch_start = 1
if opt.continue_train==1 and opt.continue_from_batch>1 then
    epoch_start = opt.continue_from_batch
end

for epoch = epoch_start, opt.niter+opt.niter_decay do
    epoch_tm:reset()
