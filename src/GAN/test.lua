...
for n = 1, math.floor(opt.how_many) do
  ...
  local tm_ = tm:time().real
  print(('Time Taken: %.3f'):format(tm_))
  if n > 10 then -- first few runs are significantly slower
    tm_avg = tm_avg + tm_
  end
...
print(('Average FPS: %.3f'):format((math.floor(opt.how_many)-10)/tm_avg))
