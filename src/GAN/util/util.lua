...
function util.normalize_corr(img)
    c = img:size(1)/2
    img_i = img:narrow(1,1,c)
    img_q = img:narrow(1,c+1,c)
    img_abs = torch.sqrt(torch.pow(img_q,2)+torch.pow(img_i,2))
    img_normalized = torch.cdiv(img, torch.cat(img_abs,img_abs,1))
    -- and select low high freq
    img_normalized_sel = torch.cat({img_normalized:narrow(1,1,1), img_normalized:narrow(1,c,1), img_normalized:narrow(1,c+1,1), img_normalized:narrow(1,c+c,1)}, 1)
    return img_normalized_sel -- [-1,1]
end

function util.corr2phase(img)
    c = img:size(1)/2
    img_i = img:narrow(1,1,c)
    img_q = img:narrow(1,c+1,c)
    img_phase = torch.atan2(img_q,img_i)
    -- and select low high freq
    img_phase = torch.cat(img_phase:narrow(1,1,1), img_phase:narrow(1,c,1), 1)
    return img_phase:div(math.pi) -- normalizing from [-pi,pi] to [-1,1] for resnet model (tanh as output activation)
end

function util.corr2abs(img)
    c = img:size(1)/2
    img_i = img:narrow(1,1,c)
    img_q = img:narrow(1,c+1,c)
    img_abs = torch.sqrt(torch.pow(img_q,2)+torch.pow(img_i,2))
